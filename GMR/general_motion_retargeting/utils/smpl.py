...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path):
    ...
    num_betas = body_model.num_betas

    betas_raw = smpl_params_global['betas'][0].numpy()
    if len(betas_raw) < num_betas:
        betas = np.pad(betas_raw, (0, num_betas - len(betas_raw)))
    else:
        betas = betas_raw[:num_betas]
    
    # NOTE: No frame correction here. GVHMR's `smpl_params_global` is already
    # in the standard "ay" (world Y-up, gravity-aligned) frame — see
    # get_smpl_params_w_Rt_v2(..., tsf="any->ay") and the demo's ground-on-Y
    # postprocess in GVHMR. The Y-up -> Z-up conversion that GMR's smplx IK
    # config requires is applied later in get_gvhmr_data_offline_fast()
    # (rotation matrix [[1,0,0],[0,0,-1],[0,1,0]], which maps +Y -> +Z).
    # An earlier Rz(pi/2) here double-rotated the body (Rz90 then Rx90) and
    # laid the robot on its side; it has been removed.

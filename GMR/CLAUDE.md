...
# GVHMR prediction (hmr4d_results.pt) to robot — the MechDance pipeline entry point
python scripts/gvhmr_to_robot.py --gvhmr_pred_file <hmr4d_results.pt> --robot unitree_g1 --save_path <output.pkl>
```
...
### Convert .pkl → CSV (for the RL pipeline)
```bash
# Reads all .pkl in <folder>, writes <folder>/csv/*.csv (root_pos 3 + root_rot 4 + dof_pos 29 = 36 cols, 30 fps)
python scripts/batch_gmr_pkl_to_csv.py --folder <dir_with_pkl_files>
```

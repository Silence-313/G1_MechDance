...
from hmr4d.utils.net_utils import get_device, get_valid_mask, repeat_to_max_len, repeat_to_max_len_dict
    ...
    def _process_data(self, data, idx):
        ...
        if False:  # render
            for idx_render in range(10):
                ...
                # render
                renderer = Renderer(width, height, device=get_device(), faces=faces, K=K_fullimg)
                ...
                for i in tqdm(range(len(verts)), desc=f"Rendering {vname}"):
                    ...
                    img_overlay_pred = renderer.render_mesh(verts[i].to(get_device()), bg, vertex_colors, VI=1)

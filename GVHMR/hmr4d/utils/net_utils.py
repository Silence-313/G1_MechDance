...
def get_device():
    """Auto-detect available device: cuda if available, else cpu."""
    return "cuda" if torch.cuda.is_available() else "cpu"


def to_cuda(data):
    """Move data in the batch to the available device (cuda/cpu), carefully handle data that is not tensor"""
    device = get_device()

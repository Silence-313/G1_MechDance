...
class PycolmapRansacTwoViewGeometrySolver:
    ...
    def solve(self, pts0, pts1):
        ...
        # pycolmap returns None when two-view geometry cannot be estimated
        # (too few inliers / degenerate motion, e.g. motion blur or a
        # featureless frame). Even when an object IS returned, cam2_from_cam1
        # (the relative pose) can still be None if pose recovery failed.
        # Assume no camera motion for this pair rather than crashing the whole
        # VO pipeline. Later pairs still accumulate on top of this identity.
        if answer is None or answer.cam2_from_cam1 is None:
            print("[SimpleVO] Two-view geometry failed for a frame pair; assuming no camera motion.")
            return np.eye(4)

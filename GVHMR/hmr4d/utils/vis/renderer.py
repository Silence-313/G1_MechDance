...
class Renderer:
    def __init__(self, width, height, focal_length=None, device=None, faces=None, K=None, bin_size=None):
...
def get_global_cameras(verts, device=None, distance=5, position=(-5.0, 5.0, 0.0)):
...
def get_global_cameras_static(
    verts, beta=4.0, cam_height_degree=30, target_center_height=1.0, use_long_axis=False, vec_rot=45, device=None
):
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    L, V, _ = verts.shape

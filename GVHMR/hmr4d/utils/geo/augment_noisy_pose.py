...
from hmr4d.utils.net_utils import get_device
...
def get_jitter_cuda(shape=(8, 120), s_jittering=5e-2):
    ...
    if "jittering" not in COCO17_AUG_CUDA:
        COCO17_AUG_CUDA["jittering"] = COCO17_AUG["jittering"].to(device).reshape(1, 1, 17, 1)
    jittering = COCO17_AUG_CUDA["jittering"]
    jittering_noise = torch.randn((*shape, 17, 3), device=device) * jittering * s_jittering
    return jittering_noise
...
def get_lfhp_cuda(shape=(8, 120), s_peak=3e-1, s_peak_mask=5e-3):
    ...
    if "peak" not in COCO17_AUG_CUDA:
        COCO17_AUG_CUDA["pmask"] = COCO17_AUG["pmask"].to(device)
        COCO17_AUG_CUDA["peak"] = COCO17_AUG["peak"].to(device).reshape(17, 1)

    pmask = COCO17_AUG_CUDA["pmask"]
    peak = COCO17_AUG_CUDA["peak"]
    peak_noise_mask = torch.rand(*shape, 17, device=device) * pmask < s_peak_mask
    peak_noise = (
        peak_noise_mask.float().unsqueeze(-1).expand(-1, -1, -1, 3) * torch.randn(3, device=device) * peak * s_peak
    )
    return peak_noise
...
def get_bias_cuda(shape=(8, 120), s_bias=1e-1):
    ...
    if "bias" not in COCO17_AUG_CUDA:
        COCO17_AUG_CUDA["bias"] = COCO17_AUG["bias"].to(device).reshape(1, 17, 1)

    bias = COCO17_AUG_CUDA["bias"]
    bias_noise = torch.randn((shape[0], 17, 3), device=device) * bias * s_bias

...
from hmr4d.utils.net_utils import get_device, repeat_to_max_len, gaussian_smooth
...
class MetricMocap(pl.Callback):
    ...
    def on_predict_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        if False:  # incam visualize debug
            ...
            renderer = Renderer(width, height, device=get_device(), faces=faces, K=K)
            ...
            for i in tqdm(range(seq_length), desc=f"Rendering {vname}"):
                img_overlay_gt = renderer.render_mesh(vertices_gt[i].to(get_device()), images[i], [39, 194, 128])
                ...
                img_overlay_pred = renderer.render_mesh(vertices_pred[i].to(get_device()), images[i])
                # img_overlay_pred = renderer.render_mesh(vertices_pred[i].to(get_device()), np.zeros_like(images[i]))
                ...
                black_overlay_obs = renderer.render_mesh(
                    vertices_cr_gt[i].to(get_device()), np.zeros_like(images[i]), colors=[39, 194, 128]
                )
                ...
                black_overlay_obs = renderer.render_mesh(
                    vertices_cr_obs[i].to(get_device()), black_overlay_obs, colors=obs_error_color[None]
                )
                black_overlay_pred = renderer.render_mesh(
                    vertices_cr_pred[i].to(get_device()), black_overlay_pred, colors=pred_error_color[None]
                )
        ...
        if False:  # Visualize incam + global results
            ...
            # two renderers
            renderer_incam = Renderer(width, height, device=get_device(), faces=faces, K=K)
            renderer_glob = Renderer(width, height, estimate_focal_length(width, height), device=get_device(), faces=faces)
            ...
            color = torch.ones(3).float().to(get_device()) * 0.8

            writer = get_writer(out_fn, fps=30, crf=23)
            for i in tqdm(range(seq_length), desc=f"Rendering {vname}"):
                # incam
                img_overlay_pred = renderer_incam.render_mesh(verts_incam[i].to(get_device()), images[i], [0.8, 0.8, 0.8])
                # if batch["meta_render"][0].get("bbx_xys", None) is not None:  # draw bbox lines
                #     bbx_xys = batch["meta_render"][0]["bbx_xys"][i].cpu().numpy()
                #     lu_point = (bbx_xys[:2] - bbx_xys[2:] / 2).astype(int)
                #     rd_point = (bbx_xys[:2] + bbx_xys[2:] / 2).astype(int)

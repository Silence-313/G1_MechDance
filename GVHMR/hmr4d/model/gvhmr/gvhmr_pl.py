...
class GvhmrPL(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        j2d_visible_mask = get_visible_mask(gt_j3d.shape[:2]).to(get_device())  # (B, L, J)
        ...
        if True:  # Set both legs as invisible for a period
            legs_invisible_mask = get_invisible_legs_mask(gt_j3d.shape[:2]).to(get_device())  # (B, L, J)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if False:  # wis3d
            ...
            smplx_model = make_smplx("rich-smplx", gender="neutral").to(get_device())
            ...
            # GT (w)
            smplx_models = {
                "male": make_smplx("rich-smplx", gender="male").to(get_device()),
                "female": make_smplx("rich-smplx", gender="female").to(get_device()),
            }

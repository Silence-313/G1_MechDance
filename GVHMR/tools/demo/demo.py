...
from hmr4d.utils.net_utils import detach_to_cpu, to_cuda, get_device
...
def render_incam(cfg):
    ...
    smplx = make_smplx("supermotion").to(get_device())
    smplx2smpl = torch.load("hmr4d/utils/body_model/smplx2smpl_sparse.pt").to(get_device())
    ...
    # renderer
    renderer = Renderer(width, height, device="cuda" if torch.cuda.is_available() else "cpu", faces=faces_smpl, K=K)
    ...
    for i, img_raw in tqdm(enumerate(reader), total=get_video_lwh(video_path)[0], desc=f"Rendering Incam"):
        img = renderer.render_mesh(verts_incam[i].to(get_device()), img_raw, [0.8, 0.8, 0.8])
...
def render_global(cfg):
    ...
    smplx2smpl = torch.load("hmr4d/utils/body_model/smplx2smpl_sparse.pt").to(get_device())
    faces_smpl = make_smplx("smpl").faces
    J_regressor = torch.load("hmr4d/utils/body_model/smpl_neutral_J_regressor.pt").to(get_device())
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        Log.info("[GPU]: No NVIDIA GPU found, running on CPU")

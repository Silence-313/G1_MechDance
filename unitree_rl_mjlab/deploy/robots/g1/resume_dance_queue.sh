#!/usr/bin/env bash
#
# resume_dance_queue.sh — 从 model_9999.pt 断点续训, 每支舞再训 10000 轮 (round 2)。
#
# 断点续训原理:
#   train.py 加 --agent.resume + --agent.load_run=<匹配第1轮run目录的正则> \
#   + --agent.load_checkpoint=model_9999.pt。runner.load() 会恢复 iter=9999,
#   再跑 max-iterations=10000 就接着训到 iter≈19998。
#
# 用法 (在训练服务器):
#   bash deploy/robots/g1/resume_dance_queue.sh                # 默认三卡 0 1 2 全上
#   GPUS="0 1" bash deploy/robots/g1/resume_dance_queue.sh     # 或指定用哪几张卡
#
set -uo pipefail
...
GPUS="${GPUS:-0 1 2}"                # 用哪些物理卡 (三卡全上)
NUM_ENVS="${NUM_ENVS:-8192}"
MAX_ITERS="${MAX_ITERS:-10000}"      # 再训 10000 轮
RESUME_CKPT="${RESUME_CKPT:-model_9999.pt}"
SESSION="${SESSION:-resume_train}"
STAGGER="${STAGGER:-30}"             # 错峰启动: 每卡领任务后多等 gpu*STAGGER 秒 (GPU0=0s/GPU1=30s/GPU2=60s)

# ============================================================
# 续训队列 (每行一个舞名, 即 npz 文件名去掉 .npz)
# ============================================================
declare -a JOBS=(
  "squat_957d1c6f"
  "squat_17fe0adf"
...
resume_one() {
  ...
  local name="${dance}_r2"              # round2 用新 run-name, 避免和 round1 目录混淆
  log "续训 motion=$motion (run=${name}) GPU=$gpu, 从 $RESUME_CKPT 再训 ${MAX_ITERS} 轮"
...
worker() {
  local gpu="$1"
  local logfile="$RL_DIR/logs/resume_worker_gpu${gpu}.log"
  local job delay
  while true; do
    job="$(pop_job)"

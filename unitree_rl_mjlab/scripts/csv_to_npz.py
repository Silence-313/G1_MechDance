...
class MotionLoader:
  ...
  def _load_motion(self):
    ...
    # No root-z compensation here. GMR's gvhmr_to_robot.py now grounds the
    # output itself (post-retarget foot-touch adjustment, GROUND_TARGET=0.02),
    # so the saved pkl/csv already has feet near z=0. A hardcoded 0.1 m
    # subtraction used to undo GMR's offset_human_data_to_ground() lift, but
    # that lift is now corrected inside gvhmr_to_robot.py and the extra 0.1
    # here pushed regenerated motions BELOW the ground (IMG_5805.npz feet at
    # ~-0.08 m). The reference is played back by setting the root to the
    # motion's root height, so an already-grounded motion needs no shift.
...
def run_sim(
  ...
  while not file_saved:
    ...
    root_states[:, :2] += scene.env_origins[: root_states.shape[0], :2]
